...
def run_NN_regression(df, plot=False, seed=None, save_dir=None):
    ...
    # train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    # val_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    # retrain best model on combined train + val sets
    ...
    input_dim = X_train.shape[1]
    final_model = MLP(input_dim=input_dim, hidden_dims=hidden_dims, dropout=dropout)
    mse, r2 = train_model(final_model, trainval_loader, test_loader, epochs=300, lr=lr, device=device)
    rmse = np.sqrt(mse)
    print(f"Final Test MSE: {mse:.4f}, R2: {r2:.4f}")
    ...
    if save_dir is not None:
        ...
        # Save best hyperparameters
        hp_file = os.path.join(run_dir, "best_hyperparameters.json")
        with open(hp_file, "w") as f:
            json.dump(best_params, f, indent=4)
        print(f"Best hyperparameters saved to {hp_file}")
        ...
        if plot:
            ...
            plt.ylabel("Predicted pCMC")
            plt.title("True vs Predicted pCMC")
            plot_file = os.path.join(run_dir, "scatter_plot.png")
            plt.savefig(plot_file, dpi=300, bbox_inches="tight")
            plt.close()
            print(f"Plot saved to {plot_file}")
    else:
